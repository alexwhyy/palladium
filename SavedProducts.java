...
public abstract class SavedProducts{
   ...
   /*
   Name: updateFile
   Purpose: Updates file when array is changed
   */
   abstract void updateFile();

   /*
   Name: addProduct
   Purpose: Removes adds Product from Search to array
   Description: passes in ArrayList and the index of the product from searchList to be added
   */
   public void addProduct(int input, ArrayList<Product> list, ArrayList<Product> searchList){
      list.add(searchList.get(input));
      updateFile();
   }
